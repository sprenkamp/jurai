...
def update_tracking_file(tracking_file, processed_files):
    with open(tracking_file, 'w') as file:
        for filename in processed_files:
            file.write(filename + '\n')

def process_rtf_files(input_folder, output_folder, tracking_file, encoding="cp1252", errors="strict"):
    processed_files = get_processed_files(tracking_file)

    for filename in os.listdir(input_folder):
        if filename.endswith(".rtf") and filename not in processed_files:
            rtf_filepath = os.path.join(input_folder, filename)
            with open(rtf_filepath, 'r', encoding='utf-8', errors='ignore') as rtf_file:
                rtf_content = rtf_file.read()
                plain_text = rtf_to_text(rtf_content, encoding=encoding, errors=errors)

            # Save plain text to output directory
            output_filename = os.path.splitext(filename)[0] + ".txt"
            output_filepath = os.path.join(output_folder, output_filename)
            with open(output_filepath, 'w', encoding='utf-8', errors='ignore') as txt_file:
                txt_file.write(plain_text)

            processed_files.add(filename)
            print(f"Processed file: {filename}")

    # Update the tracking file in the output directory
    tracking_file_in_output = os.path.join(output_folder, "tracking_file.txt")
    update_tracking_file(tracking_file_in_output, processed_files)
    print(f"Tracking file updated: {tracking_file_in_output}")

def main():
    input_directory = "/Users/therealchrisbrennan/Documents/Project/Trainingsdatei "
    output_directory = "/Users/therealchrisbrennan/Documents/Project/Trainingsdatei "

    # Ensure the output directory exists
    os.makedirs(output_directory, exist_ok=True)

    # Use the correct variable name for tracking file
    tracking_file_path = os.path.join(output_directory, "tracking_file.txt")

    process_rtf_files(input_directory, output_directory, tracking_file_path)
